...
#ifdef TARGET_SIMULATOR
    uint8_t payload[] = { 1, 103, static_cast<uint16_t>(temperature * 100) >> 8, static_cast<uint16_t>(temperature * 100) & 0xff };
    int16_t retcode = lorawan.send(15, payload, sizeof(payload), MSG_UNCONFIRMED_FLAG);
#else
